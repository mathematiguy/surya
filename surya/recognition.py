...
def batch_recognition_logits(images: List, languages: List[List[str]], model, processor, batch_size=None):
    assert all([isinstance(image, Image.Image) for image in images])
    assert len(images) == len(languages)
    ...
    # Initialize a list to store logits per image
    all_token_logits = []
    ...
    for i in tqdm(range(0, len(images), batch_size), desc="Recognizing Text"):
        ...
        inference_token_count = batch_decoder_input.shape[-1]
        batch_predictions = [[] for _ in range(current_batch_size)]
        batch_token_logits = [[] for _ in range(current_batch_size)]  # Initialize logits storage
        ...
        with torch.no_grad():  # inference_mode doesn't work with torch.compile
            ...
            while token_count < settings.RECOGNITION_MAX_TOKENS:
                ...
                for j in range(current_batch_size):
                    if not all_done[j]:
                        token_logits = logits[j, -1, :].detach().cpu()
                        batch_token_logits[j].append(token_logits)

                token_count += inference_token_count
                inference_token_count = batch_decoder_input.shape[-1]

            # Append logits of the current batch to the overall list
            all_token_logits.extend(batch_token_logits)
